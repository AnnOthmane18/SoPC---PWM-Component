 ...
 *
 * Generated: 2022-10-27 00:09:24.359
 *
 */
...
MEMORY
{
    ...
    CPU_MEM : ORIGIN = 0x00020020, LENGTH = 102368
}

    /* Define symbols for each memory base-address */
 __alt_mem_CPU_MEM = 0x00020000 ;
...
SECTIONS
{
    ...
    .text :
    {
        ...
        *(.dynamic)
        ...
        KEEP (*(.jcr))
        . = ALIGN(32 / 8);
    } >  CPU_MEM =0x3a880100 /* NOP on Nios2 (big endian) */
    ...
    .CPU_MEM LOADADDR (.bss) + SIZEOF (.bss): AT (LOADADDR (.bss) + SIZEOF (.bss))
    {
        PROVIDE (_alt_partition_CPU_MEM_start = ABSOLUTE(.));
        *(.CPU_MEM .CPU_MEM.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_CPU_MEM_end = ABSOLUTE(.));
        _end = ABSOLUTE(.);
        end = ABSOLUTE(.);
        __alt_stack_base = ABSOLUTE(.);
    } > CPU_MEM

    PROVIDE (_alt_partition_CPU_MEM_load_addr = LOADADDR(.CPU_MEM));
...
PROVIDE( __alt_heap_limit    = 0x00039000 );
